...
export async function generateQuotationPDF(quotation: QuotationWithServices) {
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  const quoteId = `Quotation: ${quotation.name}`;
  ...
  const grandTotal = subtotal - discountAmount; // 6. Pricing breakdown (No Box, Right-Aligned)
...
export async function generateQuotationPDFBase64(quotation: QuotationWithServices): Promise<string> {
  ...
  doc.setFont("helvetica", "bold");
  const quoteId = `Quotation: ${quotation.name}`;
  const quoteDate = `Date: ${new Date(quotation.created_at).toLocaleDateString("en-GB")}`;
  doc.text(quoteId, margin, currentY);
  currentY += 5;
  doc.text(quoteDate, margin, currentY);
  currentY += 10;
  ...
  if (quotation.project) {
    ...
    if (quotation.project.startDate) {
      doc.text(
        `Start Date: ${new Date(quotation.project.startDate).toLocaleDateString("en-GB")}`,
        projectStartX,
        projectY
      );
      projectY += 5;
    ...
    if (quotation.project.endDate) {
      doc.text(
        `End Date: ${new Date(quotation.project.endDate).toLocaleDateString("en-GB")}`,
        projectStartX,
        projectY
      );
      projectY += 5;
  ...
  currentY = Math.max(clientY, projectY) + 10;
  ...
  doc.setFont("helvetica", "bold");
  doc.text("TOTAL AMOUNT:", totalsX, totalsY, { align: "right" });
  doc.text(`RM ${grandTotal.toFixed(2)}`, valueX, totalsY, { align: "right" });
