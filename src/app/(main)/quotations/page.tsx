...
export default function QuotationsPage() {
  ...
  const fetchData = async () => {
    try {
      const [servicesData, quotationsData] = await Promise.all([getAllServices(), getAllQuotations()])
  ...
  const handleCreateQuotation = async () => {
    if (!quotationForm.name || !quotationForm.description || selectedServiceIds.length === 0) {
      ...
      <div className="container mx-auto p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent className="!w-[70vw] !max-w-[70vw] !max-h-[90vh] rounded-lg" showCloseButton={false}>
                        ...
                        <div key={service.id} className="flex items-start space-x-3 p-3 border rounded-lg">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-start">
                              ...
                              <Badge variant="outline">${service.basePrice.toFixed(2)}</Badge>
                            </div>
                          </div>
                        </div>
                      ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {quotations.map((quotation) => (
            <Card key={quotation.id} className="hover:shadow-md transition-shadow">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <Badge variant="outline">${quotation.totalPrice.toFixed(2)}</Badge>
                    </div>
                  </div>
                  <div className="flex space-x-1">
                    ...
                    <Button
                      ...
                      disabled={quotation.status !== "accepted" && quotation.status !== "paid"}
                    >
                      <Briefcase className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDeleteQuotation(quotation.id.toString())}>
              ...
              <CardContent>
                <CardDescription className="mb-3">{quotation.description}</CardDescription>
